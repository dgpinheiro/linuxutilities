...
my ($level, $ipdef, $cmd, $outdir, $prefix, $auxfile, $user, $password);
...
$LOGGER->logdie("Missing command to execute") unless ($cmd);
...
foreach my $ipd ( split(',', $ipdef) ) {
    ...
    do {
        ...
        if ($stdout) {
            open(OUT, ">", $outdir.'/'.$prefix.$ip->ip().'_stdout.txt') or $LOGGER->logdie($!);
            print OUT $stdout;
            close(OUT);
        }
        if ($stderr) {    
            open(ERR, ">", $outdir.'/'.$prefix.$ip->ip().'_stderr.txt') or $LOGGER->logdie($!);
            print ERR $stderr;
            close(ERR);
        }        
    } while ( ++$ip );
